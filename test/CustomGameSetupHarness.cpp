...
struct CustomGameSetupHarness {
  static void ui(const std::string &output, int control) {
    ...
    auto key = [&](SDL_Keycode code, SDL_Keymod modifiers = KMOD_NONE,
                   int repeats = 1) {
  ...
  static void model() {
    CustomGameSetup s;
    assert(s.capacity == 4 && s.activeColonies() == 4 &&
           s.controllerCount() == 4 && s.humanColony() == 0);
    ...
    assert(s.setController(0, CustomGameSetup::Shared) &&
           s.controllerCount() == 5);
    s.setCapacity(2);
    s.setCapacity(4);
    assert(s.colonies[3].alliance == alliances[3].alliance);
    assert(s.setController(3, CustomGameSetup::Human));
    assert(s.humanColony() == 3 &&
           s.colonies[0].controller == CustomGameSetup::Computer);
    GameHeader h;
    ...
    s.presetRules(1);
    assert(s.speed == 3 && s.generator.nbWorkers == 8 &&
           s.mapRevision > revision);
    s.presetRules(2);
  ...
  static void engine(const std::string &map, int control,
    ...
    assert(e.initGame(header, game, true, false, false, map) ==
           Engine::EE_NO_ERROR);
    assert(globalContainer->liveSpectating ==
           (control == CustomGameSetup::Computer));
    ...
    if (globalContainer->liveSpectating) {
      e.gui.localPlayer =
          2; // Optional viewpoint must not change controller routing.
  ...
  static void sessionReplay(const std::string &map, int control,
                            bool solo = false) {
    globalContainer->automaticEndingGame = true;
    ...
    globalContainer->automaticGameGlobalEndConditions = false;
    {
      ...
      std::filesystem::copy_file(
          map, transient, std::filesystem::copy_options::overwrite_existing);
      Engine e;
    ...
    std::cout << "PASS real match and replay playback: control " << control
              << " solo " << solo << "\n";
  }
...
int main(int argc, char **argv) {
  ...
  globals.settings.screenWidth =
      argc > 2 && std::string(argv[2]) == "large" ? 1000 : 640;
  globals.settings.screenHeight =
      argc > 2 && std::string(argv[2]) == "large" ? 700 : 480;
  ...
  if (argc > 2 && std::string(argv[2]) == "ui") {
    for (int control :
         {int(CustomGameSetup::Computer), int(CustomGameSetup::Human),
  ...
  for (int method : playableMethods) {
    ...
    for (int retry = 0; retry < 5 && !generated; ++retry) {
      ...
      generated =
          generator.generateMap(game, request) && game.teamsCount() == 4;
    }
    assert(generated);
  }
  std::cout << "PASS all " << playableMethods.size()
            << " playable generator landscapes\n";

  CustomGameSetupHarness::model();
  assert(AINames::getAISelectorText(AI::CORTEX).find("Medium") !=
         std::string::npos);
  assert(AINames::getAIProfile(AI::CORTEX).find("wheat") != std::string::npos);
  assert(AINames::getAIProfile(AI::CORTEX).find("\n\nStrengths:") !=
         std::string::npos);
  const auto dir = std::filesystem::temp_directory_path() /
                   ("glob2-setup-test-" + std::to_string(getpid()));
  std::filesystem::create_directory(dir);
  const auto map = (dir / "generated.map").string(),
             save = (dir / "match.game").string();
  {
    Game g(nullptr);
    MapGenerator generator;
    ...
    GAGCore::BinaryInputStream in(
        Toolkit::getFileManager()->openInputStreamBackend(map));
    ...
    for (int y = 0; y < g.map.getH(); ++y)
      for (int x = 0; x < g.map.getW(); ++x) {
        ...
        assert(g.map.getResource(x, y).type ==
               loaded.map.getResource(x, y).type);
      }
    for (int i = 0; i < 4; ++i) {
  ...
  CustomGameSetupHarness::sessionReplay("maps/FourSquares1.map",
                                        CustomGameSetup::Human, true);
